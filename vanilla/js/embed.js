...
window.vanilla.embed = function(host) {
		...
		if (message[0] == 'height') {
			...
			if (disablePath) {
				...
				if (currentPath != message[1]) {
					...
					currentPath = currentPath.replace('/index.php?p=', '');
					// 1
					currentPath = stripParam(currentPath, 'remote=');
					// 2
					currentPath = stripParam(currentPath, 'locale=');
					// 3
	...
	vanillaUrl = function(path) {
		...
		if (embed_type == 'comments') {
			result = '//' + host + '/discussion/embed/' + '&vanilla_identifier=' + encodeURIComponent(foreign_id) + '&vanilla_url=' + encodeURIComponent(foreign_url);
			...
			result = '//' + host + path + '&remote=' + encodeURIComponent(embedUrl) + '&locale=' + encodeURIComponent(embed_locale);
		}
		...
		// Replace the first occurrence of amp with question.
	}
